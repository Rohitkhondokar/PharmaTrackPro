$(function () {
    var canManage = window.ptCanManageSuppliers === true;

    function getToken() {
        return $('#supplierForm input[name="__RequestVerificationToken"]').val();
    }

    function dashIfEmpty(v) {
        return v ? v : '<span class="text-muted">—</span>';
    }

    var columns = [
        { data: "name" },
        { data: "contactPerson", render: dashIfEmpty },
        { data: "phone", render: dashIfEmpty },
        { data: "email", render: dashIfEmpty },
        { data: "licenseNumber", render: dashIfEmpty },
        { data: "paymentTerms", render: dashIfEmpty },
        {
            data: "isDeleted",
            render: function (isDeleted, type, row) {
                if (isDeleted) return '<span class="badge bg-secondary">Deleted</span>';
                return row.isActive
                    ? '<span class="badge bg-success">Active</span>'
                    : '<span class="badge bg-warning text-dark">Inactive</span>';
            }
        }
    ];

    if (canManage) {
        columns.push({
            data: null,
            orderable: false,
            className: "text-end",
            render: function (row) {
                if (row.isDeleted) {
                    return '<button class="btn btn-sm btn-outline-success btn-restore" data-id="' + row.id + '">' +
                           '<i class="fa-solid fa-rotate-left"></i> Restore</button>';
                }
                return (
                    '<button class="btn btn-sm btn-outline-primary btn-edit me-1" data-id="' + row.id + '">' +
                    '<i class="fa-solid fa-pen"></i></button>' +
                    '<button class="btn btn-sm btn-outline-danger btn-delete" data-id="' + row.id + '">' +
                    '<i class="fa-solid fa-trash"></i></button>'
                );
            }
        });
    }

    var table = $("#suppliersTable").DataTable({
        ajax: {
            url: "/Suppliers/GetAll",
            data: function (d) {
                d.includeDeleted = $("#chkShowDeleted").is(":checked");
            },
            dataSrc: "data"
        },
        columns: columns,
        order: [[0, "asc"]],
        responsive: true
    });

    $("#chkShowDeleted").on("change", function () {
        table.ajax.reload();
    });

    function resetForm() {
        $("#supplierForm")[0].reset();
        $("#Id").val(0);
        $("#IsActive").prop("checked", true);
        $(".text-danger.small").text("");
        $("#supplierModalTitle").text("Add Supplier");
    }

    $("#btnAddSupplier").on("click", function () {
        resetForm();
        new bootstrap.Modal("#supplierModal").show();
    });

    $("#suppliersTable").on("click", ".btn-edit", function () {
        var id = $(this).data("id");

        $.get("/Suppliers/GetById", { id: id }, function (data) {
            resetForm();
            $("#supplierModalTitle").text("Edit Supplier");
            $("#Id").val(data.id);
            $("#Name").val(data.name);
            $("#ContactPerson").val(data.contactPerson);
            $("#Phone").val(data.phone);
            $("#Email").val(data.email);
            $("#Address").val(data.address);
            $("#LicenseNumber").val(data.licenseNumber);
            $("#PaymentTerms").val(data.paymentTerms);
            $("#IsActive").prop("checked", data.isActive);
            new bootstrap.Modal("#supplierModal").show();
        }).fail(function () {
            Swal.fire("Error", "Could not load supplier details.", "error");
        });
    });

    $("#supplierForm").on("submit", function (e) {
        e.preventDefault();

        var id = parseInt($("#Id").val(), 10);
        var url = id > 0 ? "/Suppliers/Edit" : "/Suppliers/Create";

        var payload = {
            Id: id,
            Name: $("#Name").val(),
            ContactPerson: $("#ContactPerson").val(),
            Phone: $("#Phone").val(),
            Email: $("#Email").val(),
            Address: $("#Address").val(),
            LicenseNumber: $("#LicenseNumber").val(),
            PaymentTerms: $("#PaymentTerms").val(),
            IsActive: $("#IsActive").is(":checked"),
            __RequestVerificationToken: getToken()
        };

        $("#btnSaveSupplier").prop("disabled", true);

        $.post(url, payload)
            .done(function (res) {
                if (res.success) {
                    bootstrap.Modal.getInstance(document.getElementById("supplierModal")).hide();
                    table.ajax.reload();
                    Swal.fire({ icon: "success", title: res.message, timer: 1800, showConfirmButton: false });
                } else {
                    Swal.fire("Could not save", res.message, "error");
                }
            })
            .fail(function () {
                Swal.fire("Error", "Something went wrong while saving. Please try again.", "error");
            })
            .always(function () {
                $("#btnSaveSupplier").prop("disabled", false);
            });
    });

    $("#suppliersTable").on("click", ".btn-delete", function () {
        var id = $(this).data("id");

        ptConfirm("This supplier will be moved to deleted items. You can restore it later.", function () {
            $.post("/Suppliers/Delete", { id: id, __RequestVerificationToken: getToken() })
                .done(function (res) {
                    table.ajax.reload();
                    Swal.fire({ icon: res.success ? "success" : "error", title: res.message, timer: 1800, showConfirmButton: false });
                })
                .fail(function () {
                    Swal.fire("Error", "Could not delete supplier.", "error");
                });
        });
    });

    $("#suppliersTable").on("click", ".btn-restore", function () {
        var id = $(this).data("id");

        $.post("/Suppliers/Restore", { id: id, __RequestVerificationToken: getToken() })
            .done(function (res) {
                table.ajax.reload();
                Swal.fire({ icon: res.success ? "success" : "error", title: res.message, timer: 1800, showConfirmButton: false });
            })
            .fail(function () {
                Swal.fire("Error", "Could not restore supplier.", "error");
            });
    });
});
