$(function () {
    ...
    function recalcRow($row) {
        ...
        $row.find(".refund-cell").text(formatMoney(qty * price));
        recalcGrandTotal();
    }
    ...
    $(document).on("input", ".return-qty-input", function () {
        var $row = $(this).closest(".return-row");
    ...
    $("#returnForm").on("submit", function (e) {
        ...
        $(".return-row").each(function () {
            ...
            if (qty > 0) {
                rows.push({ SaleItemId: $(this).data("sale-item-id"), Quantity: qty });
            }
        });
        ...
        rows.forEach(function (item, i) {
            formData.append("Items[" + i + "].SaleItemId", item.SaleItemId);
            ...
            .done(function (res) {
                if (res.success) {
                    Swal.fire({ icon: "success", title: res.message, timer: 1500, showConfirmButton: false })
                        .then(function () {
                            window.location.href = "/SaleReturns/Details/" + res.returnId;
                        });
                } else {
